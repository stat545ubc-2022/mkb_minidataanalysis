...
As I am aiming to use the data included in cancer_sample, I am trying to
familiarize myself here. This dataset appears to identify various (mean
and sd) measures of tumor samples per patient, the measures of their
worst tumor, along with each patient’s diagnosis as M = malignant, B =
benign.
...
``` {r}
### EXPLORE HERE ###
steam_games # i dont like it, it's too messy
cancer_sample #i used to work in cancer biology before changing paths, this data makes more sense to me and is numerical which is more similar to work that i do now in computational linguistics 
apt_buildings
vancouver_trees
building_permits
parking_meters
...
quantitative data but also categorical data. Before my grad studies in
...
sense to me with cancer data and the second most understandable to me is
the trees data. Perhaps neither is relevant to my current work and I am
not an expert on either dataset but I have a good enough understanding
of this for this project I think. I don’t know much about the other
topics and find the datasets a bit too untidy.
<!----------------------------------------------------------------------------->
...
your data (only if it makes sense)) The date_planted variable was a bit
tricky to work with as a continuous variable so I created two new
variables from this variable called “year” and “age”. “Year” simply
extracts the year from the date_planted, and “age” determines what the
current age of the tree is in the present year 2022.
...
I investigated the relation between the tree’s age and the tree’s
diameter by plotting the data on a new variable called “age” as the
independent variable and “diameter” as the dependent variable.
...
passengers born in a particular year.) I am interested to see if trees
sampled from Kitsilano had a different distribution than the greater
Vancouver context does. To investigate this, I filtered in only data
that was associated with trees which came from the neighborhood of
Kitsilano.
...
What is the age distribution for the trees? To visualize this I plotted
...
An extra exercise (Point \# 1 Plot the distribution of a numeric
variable.) for curiosity… Which neighborhood has the most trees? I made
...
ggplot(vt_a, aes(x= age, y = diameter)) + geom_point() + labs(x="age", y="diameter") + facet_wrap(~genus_name)
...
#Comment: This is helpful as it pretty much confirms what the summary data told me, the age and range is pretty consistent across different neighborhoods!
```
